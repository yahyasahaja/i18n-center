...
export default function ApplicationDetailPage() {
  ...
  const handleDownloadJson = async () => {
    try {
      const blob = await exportApi.exportApplication(applicationId, undefined, appStage)
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `${currentApplication?.code ?? applicationId}_${appStage}.json`
    ...
    <Layout>
      <div className="space-y-6">
        ...
        {activeJobs && (activeJobs.add_language_jobs.length > 0 || activeJobs.translate_jobs.length > 0) && (
          <Card title="Active jobs">
            <div className="space-y-4">
              ...
              {(() => {
                // Group translate_jobs by component_id so a backfill (one job per locale)
                // shows as a single row with all pending locales listed.
                const groups = new Map<string, typeof activeJobs.translate_jobs>()
                for (const job of activeJobs.translate_jobs) {
                  const existing = groups.get(job.component_id) ?? []
                  groups.set(job.component_id, [...existing, job])
                }
                return Array.from(groups.entries()).map(([compId, jobs]) => {
                  const first = jobs[0]
                  const allLocales = jobs.flatMap((j) => j.target_locales ?? [])
                  const jobType = first.job_type.replace('_', ' ')
                  return (
                    <div key={compId} className="space-y-1">
                      <div className="flex items-center gap-2 text-sm">
                        <Loader2 className="w-3.5 h-3.5 animate-spin text-primary-600 shrink-0" />
                        <span className="font-medium text-gray-800">{first.component_name}</span>
                        <span className="text-xs text-gray-400 font-mono">{first.component_code}</span>
                        ...
                        <span className="ml-auto text-xs text-gray-400 capitalize whitespace-nowrap">
                          {jobType} · {jobs.length} locale{jobs.length !== 1 ? 's' : ''} queued
                        </span>
                      </div>
                    </div>
                  )
        ...
        <Card
          ...
          actions={
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={handleDownloadJson} title={`Download all translations (${appStage})`}>
